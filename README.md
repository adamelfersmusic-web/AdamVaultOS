...
A dark-mode studio for Adam's personal
[Parachute](https://parachute.computer) vault — a fork of
[AtelierVaultOS](https://github.com/adamelfersmusic-web/AtelierVaultOS)
repointed at the `adam` vault. Its headline surface is **Pages**: the TipTap
block editor opening Adam's real notes (`Amanda/`, `Atelier/`, …) as blocks —
never raw markdown. It also carries the **Scripts** database
(`content/scripts/…`) inherited from the original, with three lenses over one
dataset: **Table · Board · Gallery**. Full read/write over the vault's REST API.
