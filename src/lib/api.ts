...
export class VaultApi {
  ...
  /**
   * Lean list (no content) of notes across the WHOLE vault, optionally
   * excluding one or more tags server-side (e.g. `task`). Powers the Pages
   * sidebar's "all my real notes" listing — the knowledge layer — so the
   * productivity layer (tag:task) stays on its own board, never here.
   */
  async listAll(
